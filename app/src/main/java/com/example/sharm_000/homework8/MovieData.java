...
public class MovieData implements Serializable {
    ...
    public static String PHP_SERVER = "http://rohitshivsharma.com/";
    ...
    public void downloadMovieDataJson(String url) {
        ...
        if (json != null && !"".equals(json) && !"null".equals(json)) {
            ...
            parts[0] = parts[0].replaceFirst(String.valueOf(parts[0].charAt(0)), "");
            parts[parts.length - 1] = parts[parts.length - 1].replace("]", "");
            parts[parts.length - 1] = parts[parts.length - 1].substring(0, parts[parts.length - 1].length() - 1);
            JsonParser parser = new JsonParser();
            for (int j = 0; j < parts.length; j++) {
                parts[j] = parts[j].replace("\\", "");
                JsonObject obj = parser.parse(parts[j]).getAsJsonObject();
                String id = obj.get("id").getAsString();
                String name = obj.get("name").getAsString();
                name = name.replaceAll("u0027", "'");
                String desc = obj.get("description").getAsString();
                desc = desc.replaceAll("u0027", "'");
                double rating = obj.get("rating").getAsDouble();
                String imgUrl = obj.get("url").getAsString();
                Bitmap movieImg = MyUtility.downloadImageusingHTTPGetRequest(imgUrl);
                /*Bitmap movieImg;
				final Bitmap bitmap = mImgMemoryCache.get(imgUrl);
				if(bitmap!=null){
					movieImg = bitmap;
				}else{
					movieImg = MyUtility.downloadImageusingHTTPGetRequest(imgUrl);
					if(movieImg!=null){
						mImgMemoryCache.put(imgUrl,movieImg);
					}
				}*/
                this.moviesList.add(createMovie(id, name, movieImg, desc, null, null, rating, null, null, null));
            }
        }
    }

    public HashMap<String, ?> downloadMovieDataDetailsJson(String url) {
        String jsonString = MyUtility.downloadJSONusingHTTPGetRequest(url);
        Gson gson = new Gson();
        String json = gson.toJson(jsonString);
        if (json != null && !"".equals(json) && !"null".equals(json)) {
            ...
            JsonObject obj = null;
            try {
                obj = parser.parse(parts[0]).getAsJsonObject();
            } catch (Exception jsonEx) {
                Log.d("Error in json parsing: ", jsonEx.getMessage());
            ...
            String length = obj.get("length").getAsString();
            double rating = obj.get("rating").getAsDouble();
            ...
            HashMap<String, ?> movieMap = (HashMap<String, ?>) createMovie(id, name, movieImg, desc, year, length, rating, director, stars, imgUrl);
            return movieMap;
        }
        return null;
    }
    ...
    private HashMap createMovie(String id, String name, Bitmap image, String description, String year,
